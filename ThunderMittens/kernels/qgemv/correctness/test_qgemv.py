...
Oracle: out = dequantize(Wq) @ x, x is (K, 1). Also checks that tk.qgemm routes M==1 here.
...
SHAPES = [(32, 32), (128, 128), (256, 512), (1024, 256)]  # (N, K)


@pytest.mark.parametrize("shape", SHAPES)
def test_qgemv_q8_0(shape):
    N, K = shape
    rng = np.random.default_rng(0)
    W = (rng.standard_normal((N, K)) * 0.3).astype(np.float32)
    x = rng.standard_normal((K, 1)).astype(np.float32)
    Wq = quantize_q8_0(W)
    got = qgemv(mx.array(Wq), mx.array(x).astype(mx.float16), format="q8_0")
    mx.eval(got)
