...
void check_attn_correctness(id<MTLBuffer> _o_ref, id<MTLBuffer> _o_res, int B, int H, int N, int D) {
    ...
    NSString* basePath = [NSString stringWithFormat:@"%@/tests/correctness/attention/attn_fwd/", currentPath];
    for (int b = 0; b < B; b++) {
    for (int h = 0; h < H; h++) {
    for (int n = 0; n < N; n++) {
    for (int d = 0; d < D; d++) {
...
int main(int argc, const char * argv[]) {
    NSString *currentPath = [[NSFileManager defaultManager] currentDirectoryPath];
    NSString *filePath = [NSString stringWithFormat:@"%@/tests/correctness/attention/attn_fwd/randn_4_4_1024_128.txt", currentPath];
    NSLog(@"%@", filePath);
    @autoreleasepool {
