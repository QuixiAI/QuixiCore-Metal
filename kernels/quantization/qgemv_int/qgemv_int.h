...
class QGemvW2A8V2 : public Primitive {
 public:
  explicit QGemvW2A8V2(Stream stream) : Primitive(stream) {};
  void eval_cpu(const std::vector<array>&, std::vector<array>&) override;
  void eval_gpu(const std::vector<array>&, std::vector<array>&) override;
  std::vector<array> jvp(const std::vector<array>&, const std::vector<array>&,
                         const std::vector<int>&) override;
  ...
  std::pair<std::vector<array>, std::vector<int>> vmap(
      const std::vector<array>&, const std::vector<int>&) override;
  const char* name() const { return "QGemvW2A8V2"; }

  void print(std::ostream& os) override { os << "QGemvW2A8V2"; }
  bool is_equivalent(const Primitive&) const override { return true; }
  void eval(const std::vector<array>&, std::vector<array>&);
};
